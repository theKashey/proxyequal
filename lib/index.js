...
var prepareObject = function prepareObject(state) {
  var unfreezed = Object.assign({}, state);
  return unfreezed;
};
...
function proxyfy(state, report) {
  ...
  var theBaseObject = Array.isArray(state) || isProxyfied(state) ? state : prepareObject(state);

  var proxy = new ProxyConstructor(theBaseObject, {
    get: function get(target, prop) {
      ...
      if (typeof prop === 'string') {
        ...
        var type = typeof storedValue === 'undefined' ? 'undefined' : _typeof(storedValue);
    ...
    ownKeys: function ownKeys() {
      ...
      return [].concat(_toConsumableArray(Object.getOwnPropertyNames(state)), _toConsumableArray(Object.getOwnPropertySymbols(state)), [__proxyequal_scanEnd]);
    }
  });
...
var proxyState = function proxyState(state) {
  ...
  var onKeyUse = function onKeyUse(key, location) {
    if (sealed) {
      return;
    }
